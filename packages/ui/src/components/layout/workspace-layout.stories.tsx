...
import {
  ...
  type TreeNodeData,
  type ContentType,
} from '@/components/features/sidebar/sidebar';
import { WritingEditor } from '@/components/features/editor/writing-editor';
import {
  PlotEditor,
  type PlotEditorData,
} from '@/components/features/editor/plot-editor';
import {
  CharacterEditor,
  type CharacterEditorData,
} from '@/components/features/editor/character-editor';
import { MemoEditor } from '@/components/features/editor/memo-editor';
import {
  ProjectEditor,
  type ProjectEditorData,
} from '@/components/features/editor/project-editor';
import {
  EditorTabs,
  type EditorTab,
} from '@/components/features/editor/editor-tabs';
...
import {
  ...
  type AiMode,
  type Conversation,
} from '@/components/features/ai-panel/ai-panel';
                ...
                diffs: [
                  {
                    ...
                    before: '',
                    after:
                      '今日から始まる新しい冒険への期待と不安が入り混じった複雑な感情が、胸の奥で渦を巻いていた。',
                  },
                ],
...
const mockPlotData: Record<string, PlotEditorData> = {
  ...
  '1-2': {
    name: 'キャラ動線',
    synopsis: '各キャラクターの動きと合流ポイントを整理',
    structure: '主人公→村出発→森→街道→王都\nヒロイン→王都で待機→主人公と合流',
  },
  '2': {
    name: '全体構成',
    synopsis:
      '全5章構成。第1章〜第3章が前半、第4章〜第5章が後半。\n\n前半は仲間集めと世界観の提示、後半はクライマックスと解決。',
  },
};
...
function InteractiveDemo() {
  ...
  const [projectData, setProjectData] =
    useState<ProjectEditorData>(mockProjectData);
    ...
    setOpenTabs((prev) => {
      ...
      if (closing?.active && next.length > 0) {
        ...
        setSelectedNode({
          id: newActive.id,
          name: newActive.name,
          type: newActive.type as ContentType,
          nodeType: 'file',
        });
        ...
        <ProjectEditor
          ...
          onChange={(field, value) => {
            ...
            if (field === 'name') {
              const tabId = 'project:settings';
              setOpenTabs((prev) =>
                prev.map((t) => (t.id === tabId ? { ...t, name: value } : t)),
              );
            }
          }}
        />
